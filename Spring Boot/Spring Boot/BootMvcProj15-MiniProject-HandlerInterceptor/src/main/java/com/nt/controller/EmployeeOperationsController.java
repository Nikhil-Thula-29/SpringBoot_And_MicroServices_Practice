...
public class EmployeeOperationsController {
	...
	public String processRegisterEmployeeFormPage(RedirectAttributes attr, //RedirectAttributes are used to pass resultmsg to get method as map is request scope
		...
		//enable server side form validations only when the client side form validations are not done
		...
		if(serv.isDesignRejectList(emp.getJob())) {
	...
	public String showReportByPagination(Map<String,Object> map,
										@PageableDefault(page=0,size=3,sort={"job","empno"},direction=Sort.Direction.ASC) Pageable pageable) {
		//get requested page data
		Page<Employee> page=serv.showEmployeeByPagination(pageable);
